...
tag_post = Table(    # таблица tag_post
    "tag_post",
    Base.metadata,
    Column("post_id", Integer, ForeignKey("post.id")),   # связь 1 ко многим
    Column("tag_id", Integer, ForeignKey("tag.id"))      # связь 1 ко многим
)


class Post(Base):   # таблица "post"
    __tablename__ = "post"
    ...
    tags = relationship("Tag", secondary=tag_post, backref="posts")   # связь 1 ко многим
